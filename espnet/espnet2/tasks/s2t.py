...
# continual learning
from espnet2.legacy.nets.pytorch_backend import continual_learning3 as cl
...
cl_choices = ClassChoices(
    "cl_method",
    classes=dict(
        finetuning_linear=cl.FineTuningLinear,
    ),
...
class S2TTask(AbsTask):
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetS2TModel:
        ...
        if args.cl_method is not None:
            ...
            cl_conf = args.cl_method_conf or {}
            cl_method = cl_method_class(
                device=device,
                **cl_conf,
            )
        else:
        ...
        if cl_method is not None and hasattr(cl_method, "set_model"):
            cl_method.set_model(model)
